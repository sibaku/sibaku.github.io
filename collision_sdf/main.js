...
class Rectangle extends Primitive {
  ...
  sdf(p) {
    ...
    return (
      Math.min(jm.max(d), 0.0) +
      jm.norm(jm.cwiseMax(d, jm.setZero(jm.similar(d))))
    );
  }
...
function handlePair(
  ...
  primA,
  primB,
  { drawInterBounds, drawInitPoints, drawFinalPoints, drawConnection, drawGrad }
) {
  ...
  for (let i = 0; i < pN; i++) {
    for (let j = 0; j < pM; j++) {
      const p = jm.add(
        smin,
        jm.cwiseMult(sdelta, v32.from([i / (pN - 1), j / (pM - 1)]))
      );
      let idx = i + pN * j;
  ...
  const collisions = [];
  jm.colwise(pointsInitial, (col, j) => {
    const { intersectDist, unionDist, p, g } = iterateIntersection(
      col,
      primA,
      primB,
      {}
    ...
    if (intersectDist <= 1) {
      // collision
      collisions.push(
        new Collision(primA, primB, p, grad(primB, p), unionDist)
      );
    }
  });
...
function updatePrimitive(prim, dt) {
  ...
  jm.scale(prim.vel, 0.99, prim.vel);
  prim.angVel *= 0.99;
}
...
function computeImpulse(c) {
  ...
  const invI =
    Math.pow(cross2D(ra, n), 2) * primA.invInertia +
    Math.pow(cross2D(rb, n), 2) * primB.invInertia;
...
function drawVelocity(
  ctx,
  prim,
  { scale = 1, strokeStyle = "rgb(128,0,128)" }
) {
  ctx.save();
  ctx.strokeStyle = strokeStyle;
...
class Penetration {
  ...
  resolve(epsilon = 0.25) {
    ...
    jm.add(
      this.primA.pos,
      jm.scale(this.dir, factor * this.primA.invMass),
      this.primA.pos
    );
    jm.add(
      this.primB.pos,
      jm.scale(this.dir, -factor * this.primB.invMass),
      this.primB.pos
    );
  }
}
document.body.onload = () => {
  ...
  primitives.push(
    new Rectangle({
      halfSize: v32.from([100, 150]),
      pos: v32.from([700, 300]),
      angle: jm.deg2rad(45),
      angVel: jm.deg2rad(10),
      mass: 50,
    })
  );
  primitives.push(
  ...
  primitives.push(
    new Rectangle({
      halfSize: v32.from([50, 50]),
      pos: v32.from([200, 400]),
      vel: v32.from([10, -40]),
      angVel: jm.deg2rad(10),
    })
  );
  ...
  primitives.push(
    new Rectangle({
      halfSize: v32.from([canvas.width, 10]),
      pos: v32.from([0, canvas.height - 20]),
      mass: 0.0,
      inertia: 0,
    })
  );
  ...
  primitives.push(
    new Rectangle({
      halfSize: v32.from([10, canvas.height]),
      pos: v32.from([20, 0]),
      mass: 0.0,
      inertia: 0,
    })
  ...
  const [checkInterDrawBounds, labelInterBounds] = checkbox(
    "InterBounds",
  ...
  const [checkDrawPointsInit, labelDrawPointsInit] = checkbox(
    "InitPoints",
    "Draw initial points",
    false
  );
  ...
  const [checkDrawPointsAfter, labelDrawPointsAfter] = checkbox(
    "AfterPoints",
    "Draw final points",
    true
  );
  document.body.appendChild(
    group([checkDrawPointsAfter, labelDrawPointsAfter])
  );

  const [checkDrawConnection, labelDrawConnection] = checkbox(
    "Connection",
  ...
  const [checkDrawGradient, labelDrawGradient] = checkbox(
    "Grad",
    "Draw collision gradient",
    true
  );
  document.body.appendChild(group([checkDrawGradient, labelDrawGradient]));

  const [checkDrawVelocity, labelDrawVelocity] = checkbox(
    "Velocity",
    "Draw velocity",
    true
  );

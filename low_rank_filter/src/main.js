...
function makeUserInterfaceApprox(container) {
    container.innerHTML = "";
    ...
    containerInter = createContainer(
        [
            sliderDescription,
            slider
        ]
...
function makeUserInterfaceFilter(container) {
    ...
    }
    {
        ...
        header.innerHTML = "Low-rank approximation";
    ...
    options.push({
        ...
        f: () => {
            const g = generateStar(71, 0.45, 0.9, 4);
            return jsm.scale(g, 1.0 / jsm.sum(g), g);
    ...
    optionsImg.push({
        text: "Cat",
        src: App.resourcePath("img/cat_small.jpg")
    });
    optionsImg.push({
        text: "Pumpkin",
        src: App.resourcePath("img/pumpkins.jpg")
...
function gaussianSVDExploreUpdate(container, state) {
    M = generateGaussian(15, state.sigma);
    svd = jsm.computeSVD(M);


    u = jsm.scale(jsm.col(svd.U, 0), Math.sqrt(svd.S.at(0)));
    v = jsm.scale(jsm.transpose(jsm.col(svd.V, 0)), Math.sqrt(svd.S.at(0)));
...
function gaussianSVDExplore(container) {

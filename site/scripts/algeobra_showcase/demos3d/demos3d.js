...
import * as THREE from "../../bundles/three.bundle.min.js";


import { OrbitControls } from "../../bundles/three.bundle.min.js";
...
function showFrustum(containerId) {
    ...
    alg.registerAlgeobraIntersectors();
    ...
    camera.position.copy(new THREE.Vector3(3, 5, 0));
    ...
    const algeoThree = new alg.AlgeobraThreeScene(scene, threeScene);
    ...
    for (let i = 0; i < n; i++) {
        ...
        const points1 = points0.map(o => scene.add(new DefFunc(deps => {
            ...
            return alg.makePoint3(new THREE.Vector3(p4.x, p4.y, p4.z));
        ...
        const pointsInter = points1.map((o, i) => scene.add(new DefFunc(deps => {
            ...
            const { p: q } = deps.b;
            const { t } = deps;
            return alg.makePoint3(p.clone().lerp(q, t.value));

import { EMPTY_INFO, EMPTY, Vec2 } from "../../bundles/algeobra.bundle.min.js";
...
function demoMinkowskiDifference(containerId) {
    const canvas = makeCanvas(400, 400);
    canvas.classList.add("tutCanvas");
    ...
    const manip = alg.PointManipulator.createForPoints(scene, diagram.coordinateMapper, canvas, [...pointsA, ...pointsB], 40);
...
function demoInscribedAngle(containerId) {
    ...
    const manip = alg.PointManipulator.createForPointsAndHandles(scene, diagram.coordinateMapper, canvas,
        points, 40);
...
function demoTriangleCircles(containerId) {
    ...
    const checkOut = makeCheckboxNoLabel(scene.get(enableMid).value.value);
    checkOut.onchange = e => {
    ...
    const checkIn = makeCheckboxNoLabel(scene.get(enableIn).value.value);
    checkIn.onchange = e => {
...
function demoDeCasteljau(containerId) {
...
function demoLens(containerId) {
    ...
    const diagram = new alg.DiagramCanvas({ x0: -1, y0: -3, x1: 6, y1: 3, flipY: true, canvas });
...
function demoArcTangents(containerId) {
    ...
    const diagram = new alg.DiagramCanvas({ x0: -4, y0: -4, x1: 4, y1: 4, flipY: true, canvas });
...
function demoCurveTangentNormals(containerId) {
    ...
    const diagram = new alg.DiagramCanvas({ x0: -5, y0: -5, x1: 5, y1: 5, flipY: true, canvas });
    ...
    const checkNormNormals = makeCheckboxNoLabel(scene.get(normalizeNormals).value.value);
    ...
    const checkNormTangents = makeCheckboxNoLabel(scene.get(normalizeTangents).value.value);
...
function demoDetectGround(containerId) {
    ...
    const diagram = new alg.DiagramCanvas({ x0: -2, y0: -2, x1: 7, y1: 6, flipY: true, canvas });

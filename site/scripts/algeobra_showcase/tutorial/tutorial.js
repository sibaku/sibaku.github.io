...
import {
    EMPTY_INFO, EMPTY, Vec2,
    objectToString,
    DefText
} from "../../bundles/algeobra.bundle.min.js";
...
function controllableRectangle(containerId) {
    ...
    const diagram = new alg.DiagramCanvas({ x0: -3, y0: -3, x1: 3, y1: 3, flipY: false, canvas });
...
function reflectionRefraction(containerId) {
    ...
    const diagram = new alg.DiagramCanvas({ x0: -2, y0: -1.5, x1: 2, y1: 1.5, flipY: true, canvas });
    // this object will take care of drawing/redrawing our scene when anything changes
    // we don't use a background
    // we also let the painter take care of resizing the canvas to fill out the screen and adjusting the diagram accordingly
    const diagPainter = new alg.DiagramPainter(scene, diagram, {
...
function pythagoras(containerId) {
...
function sat(containerId) {
    ...
    const manip = alg.PointManipulator.createForPointsAndHandles(scene, diagram.coordinateMapper, canvas, handleMoves, 40);
...
function smoothFunction(containerId) {
    ...
    // we pass in the min/max corners of the coordinate system viewport that we want to see
    // we will flip y, as otherwise there might be some confusion when defining directions due to the canvas being a left-handed system
    const diagram = new alg.DiagramCanvas({ x0: 0, y0: -2, x1: 2 * Math.PI, y1: 2, flipY: true, canvas });
    // this object will take care of drawing/redrawing our scene when anything changes
...
function coordinateSystems(containerId) {
    ...
    const checkboxNormU = makeCheckboxNoLabel(scene.get(normalizeU0).value.value);
    checkboxNormU.oninput = () => {
        scene.update(normalizeU0, new DefBoolean(checkboxNormU.checked));
    };
    const checkboxNormV = makeCheckboxNoLabel(scene.get(normalizeV0).value.value);

...
document.body.onload = () => {
  ...
  const update = () => {
    ...
    ctx.font = "32px serif";
    ctx.fillText(
      `Current mode: ${mode === 0 ? "FABRIK" : "Gradient descent"}`,
      10,
      50
    );
    ctx.font = "20px serif";
    ctx.fillText("Switch mode by pressing 'm'", 10, 80);
    ctx.fillText("Reload for new random arm", 10, 110);

    ctx.beginPath();
    ctx.arc(target.at(0), target.at(1), 4, 0, 2 * Math.PI);
    ctx.fill();

...
I'm currently working a lot with distancefields. Distancefields can be described pretty easily. Given some kind of object, a distancefield ($$\operatorname{s} $$) will tell you for every point in space, how far away the next point of an object is. The first thing to note is, that the object can now characterized by all points in space, for which the distance is zero.
...
$$\operatorname{s} : \mathbb{R}^n \rightarrow \mathbb{R} $$: Distancefield 
$$\nabla\operatorname{s}$$: Gradient of the distancefield 
...
2. $$ \nabla\operatorname{s}(\mathbf{P}) \parallel \mathbf{v}$$ 
3. $$ \left\lVert\nabla\operatorname{s}(\mathbf{P})\right\rVert = 1 $$
4. $$ \mathbf{C} = \mathbf{P} - \operatorname{s}(\mathbf{P})\nabla\operatorname{s}(\mathbf{P})$$
...
This property says, that the gradient of the distancefield $$\operatorname{s} $$ at $$ \mathbf{P} $$ is parallel to the connecting vector $$ \mathbf{v} $$. The gradient is the vector of all partial derivatives. So for a 2D field, the distancefield will also be 2D (same for any dimension). One important property of the gradient is, that it will point in the direction of greatest increase. So if we find that direction, we know where the gradient points to!
...
$$ \lim_{h\rightarrow 0) \frac{\operatorname{s}(\mathbf{P} + h\mathbf{u}) - \operatorname{s}(\mathbf{P})}{h} = \nabla\operatorname{s}(\mathbf{P})\cdot \mathbf{u] = \left\lVert\nabla\operatorname{s}(\mathbf{P})\right\rVert \left\lVert\mathbf{u}\right\rVert \cos \alpha  = \left\lVert\nabla\operatorname{s}(\mathbf{P})\right\rVert \cos \alpha$$ 
...
$$ \lim_{h\rightarrow 0) \frac{\operatorname{s}(\mathbf{P} + h\^{\mathbf{v}}) - \operatorname{s}(\mathbf{P})}{h} =  = \left\lVert\nabla\operatorname{s}(\mathbf{P})\right\rVert$$ 

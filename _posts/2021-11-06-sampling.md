...
1. Generate a uniform random number uin the interval $[0,1]$
...
2. The probability density function $p$ is dependant on $x_1, x2$, so $p(x_1,x_2)$
    1. Calculate the marginal distribution $p_{x_1}(x1)= \int_{\Omega_2}p(x1,x2)dx_2$
    2. Calculate the conditional probability distribution of $x_2$ with respect to $x_1$ with the Bayes theorem: $p(x2|x1)=p(x_1,x_2)p_{x_1}(x_1)$
...
$p(\alpha | r) = \frac{p(r,\alpha)}{p_r(r)} = \frac{rR^2}{\pi R^2 2r} = \frac{1}{2\pi}$

#### Compute CDFs

$P_r(r) = \int_0^r p_r(r')dr' = \int_0^r \frac{2r'}{R^2}dr' = \frac{2}{R^2}[\frac{r^2}{2}]_0^r = \frac{r^2}{R^2}$

$P(\alpha | r) = \int_0^\alpha p(\alpha' | r)d\alpha' = \int_0^\alpha \frac{1}{2\pi}d\alpha' = \frac{\alpha}{2\pi}$
...
$p(\phi | \theta) = \frac{p(\theta,\phi)}{p_\theta(\theta)} = \frac{\sin\theta}{2\pi\sin\theta} = \frac{1}{2\pi}$

#### Compute CDFs

$P_\theta(\theta) = \int_0^\theta \sin\theta' d\theta' = [-\cos(\theta')]_0^\theta = \cos 0 - \cos\theta = 1 - \cos\theta$

$P(\phi | \theta) = \int_0^\phi p(\phi' | \theta)d\phi' = \int_0^\phi \frac{1}{2\pi}d\phi' = \frac{\phi}{2\pi}$

#### Invert CDFs

$u_1 = P_\theta(\theta) = 1 - \cos\theta \Rightarrow \cos\theta = 1 - u_1 \Rightarrow \theta = \cos^{-1}(1- u_1)$

$u_2 = P(\phi | \theta) = \frac{\phi}{\theta} \Rightarrow 2\pi u_2 = \phi$
...
$p(\phi | \theta) = \frac{p(\theta,\phi)}{p_\theta(\theta)} = \frac{(n+1)\cos^n\theta\sin\theta d}{d({\phi_{max}} - {\phi_{min}}) (n+1)\cos^n\theta\sin\theta}= \frac{1}{{\phi_{max}} - {\phi_{min}} }$
...
$P(\phi | \theta) = \int_{\phi_{min}}^{\phi}p(\phi ' | \theta)d\phi' = \int_{\phi_{min}}^{\phi} \frac{1}{{\phi_{max}} - {\phi_{min}} }d\phi' =\frac{\phi}{\phi_{max} - \phi_{min}}$
...
$p(\phi | \theta) = \frac{p(\theta,\phi)}{p_\theta(\theta)} = \frac{p(\theta,\phi)}{2\pi p(\theta,\phi)} = \frac{1}{2\pi}$
...
$P(\phi | \theta) = \int_{0}^{\phi}p(\phi ' | \theta)d\phi' = \int_{0}^{\phi} \frac{1}{2\pi }d\phi' =\frac{\phi}{2\pi}$

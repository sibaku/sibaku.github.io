...
const BlendEquation = { ADD: 1, SUBTRACT: 2, REVERSE_SUBTRACT: 3, MIN: 4, MAX: 5 };
const BlendFunction = {
    ZERO: 1,
    ONE: 2,
    SRC_COLOR: 3,
...
class RasterizerFinal {
     ...
     */
    blend_colors(source, destination, color_const,
        source_function, destination_function, blend_equation) {
        const f_source =
            this.get_blend_factor({ source, destination, color_const, blend_function: source_function });
        const f_dest = this.get_blend_factor({
        ...
        switch (blend_equation) {
            ...
            case BlendEquation.SUBTRACT:
                return sub(cwiseMult(source, f_source), cwiseMult(destination, f_dest));
            case BlendEquation.REVERSE_SUBTRACT:
                return sub(cwiseMult(destination, f_dest), cwiseMult(f_source, source));
            case BlendEquation.MIN:
                return cwiseMin(source, destination);
            case BlendEquation.MAX:
                return cwiseMax(source, destination);
            default:
                return vec4(0, 0, 0, 0);
        }
    }
     ...
     * @param blendFunction The blend function to be used
     * @return The blend factor
     */
    get_blend_factor({ source,
        destination,
        ...
        switch (blend_function) {
            ...
            case BlendFunction.ONE_MINUS_SRC_COLOR:
                return sub(vec4(1.0, 1.0, 1.0, 1.0), source);

            case BlendFunction.DST_COLOR:
                return destination;

            case BlendFunction.ONE_MINUS_DST_COLOR:
                return sub(vec4(1.0, 1.0, 1.0, 1.0), destination);
            ...
            case BlendFunction.DST_ALPHA:
                return vec4(destination.at(3), destination.at(3), destination.at(3), destination.at(3));
            case BlendFunction.ONE_MINUS_DST_ALPHA:
                return sub(vec4(1.0, 1.0, 1.0, 1.0), vec4(destination.at(3), destination.at(3), destination.at(3), destination.at(3)));

            case BlendFunction.CONSTANT_COLOR:
                return color_const;

            case BlendFunction.ONE_MINUS_CONSTANT_COLOR:
                return sub(vec4(1.0, 1.0, 1.0, 1.0), color_const);

            case BlendFunction.CONSTANT_ALPHA:
                return vec4(color_const.at(3), color_const.at(3), color_const.at(3), color_const.at(3));

            case BlendFunction.ONE_MINUS_CONSTANT_ALPHA:
                return sub(vec4(1.0, 1.0, 1.0, 1.0), vec4(color_const.at(3), color_const.at(3), color_const.at(3), color_const.at(3)));

            case BlendFunction.SRC_ALPHA_SATURATE:
                {
                    const c = Math.min(source.at(3), 1.0 - destination.at(3));
                    return vec4(c, c, c, c);
                }
            default:
                return vec4(0.0, 0.0, 0.0, 0.0);
 ...
 */
    ...
    */
    ...
    rasterize_triangle(pipeline, v0, v1,
        ...
        // useful for checking rasterization rules
        let points = [v0, v1, v2];
        ...
        for (let y = ibmin.at(1); y <= ibmax.at(1); y++) {
            for (let x = ibmin.at(0); x <= ibmax.at(0); x++) {
                ...
                // check if any coordinate is outside with a small epsilon
                const epsilon = 1E-8 * area_tri;

                if (av1 + epsilon < 0.0 || av2 + epsilon < 0.0 || av1 + av2 - epsilon > area_tri) {
                    continue;
                }
                ...
                // check for rasterization rules

                let skip_rasterization = false;
                for (let i = 0; i < 3; i++) {
                    // one point has zero weight -> lies on opposite edge
                    if (Math.abs(b.at(i)) < epsilon) {

                        const p0 = points[(i + 1) % 3];
                        const p1 = points[(i + 2) % 3];
                        const e = sub(p1, p0);
                        // check top
                        // top edge is horizontal and due to counter clockwise
                        // definition runs from left to right
                        if (Math.abs(e.at(1)) < epsilon && e.at(0) < 0.0) {
                            skip_rasterization = true;
                            break;
                        }

                        // check left
                        // left edge has to go up
                        if (e.at(1) < 0.0) {
                            skip_rasterization = true;
    ...
    process_triangle(pipeline, v0, v1, v2,
        ...
        if (culling_options.enabled) {
            ...
            // we can disregard the * 0.5 factor, since we are only concerned with
            // the sign
...
function create_culling_options({ enabled = false,
